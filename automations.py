...
def create_automation(
    type: int, url: str, location: str, name: str, supplier_id: str
) -> int | None:
    return insert_to_db(
...
def delete_automation(
    automation_id: str, supplier_id: str
) -> List[str] | str | None:
    delete_from_db(
        "DELETE FROM steps WHERE automation_id = ?", (automation_id,)
    )
...
def set_automation_schedule(automation_id: str, schedule: str) -> None:
    return insert_to_db(
        "UPDATE automations SET schedule = (:schedule) WHERE id = (:id)",
        {"schedule": schedule, "id": automation_id},
    )
...
def get_automation_card_data(automation_id: str) -> str | None:
    return query_db(
        "SELECT a.id as automation_id, s.name as supplier_name, a.type, a.name as automation_name FROM automations a JOIN suppliers s ON a.supplier_id = s.id WHERE a.id = ?",
        (automation_id,),
    )
